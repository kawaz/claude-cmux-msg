# DR-0015: 永続宛先 (cwd / repo / label) の inbox を sid 宛と並列に持つ (sid 揮発性への対応)
...
### 1. 識別子モデル: sid 主体は維持、cwd / repo / label を**並列 primitive** として追加

DR-0004 の「sid 主体」を覆さない。sid 宛は依然として「特定セッション宛 (= 個人宛、文脈共有済の人宛)」。
cwd / repo / label 宛は「**役割宛 (= プロジェクトに住んでる誰か宛)**」として並列に動作。
...
| label 宛 (新規) | "ラベル付き役割" 宛 | 永続 (任意ラベル文字列) | 「`role=maintainer` 持ち」 |
...
3. repo 宛 box (= `repo-hash` で索引)
4. label 宛 box × 自分が持つ label の数 (= `label-name` で索引)

共有 box (cwd / repo / label) は **複数 alive claude が同一 box を見ている可能性** がある:
...
宛先指定の排他: `<sid>` / `--cwd` / `--repo` / `--label` のいずれか **1 つ**を指定 (= 1 通 1 宛先)。`--text` は本文指定の別 axis (DR-0014)。

### 6. cwd / repo の正規化

- `cwd-hash` / `repo-hash` は正規化済パスの SHA-256 prefix (16 char)
- 正規化: `path.resolve` → `fs.realpathSync` (シンボリックリンク追従) → 末尾 `/` 剥がし
- worktree は別 cwd として別 box、同 repo 別 worktree は `--repo` 宛で合流 (= kawaz の `git-workflow.md` 運用と整合)
...
| **cwd / repo / label 宛 send** (新規) | 共有 box に 1 通 → alive な誰か 1 人が消化 | 永続 (alive ゼロでも未来の claude が拾う) | 「役割宛キュー」 |

両者は別 primitive。broadcast を queue semantics に変えない。

### 8. cross-home 境界 (DR-0005 補強)

- cwd / repo / label 宛も home 壁の対象 (= `--ignore-cross-home` 不在時は別 home 宛を warning)
...
- **sid 宛箱の引き継ぎ (= 新 sid が古い sid の inbox を吸収)**: 「特定の個人宛」semantics を壊す。新 sid は別人とみなすのが筋。sid 宛揮発性の解は cwd / repo / label 宛の永続軸で持つ。
...
- `src/lib/inbox.ts` / `src/lib/message.ts`: 受信箱の種類 (sid / cwd / repo / label) を msg メタに記録

### Hook

- `src/hooks/session-start.ts`: cwd / repo / labels を解決して session row に書く (具体は DR-0016)
...
- DR-0005 (home 壁): 維持、cwd / repo / label 宛も home 壁の対象
