...
しかし DR-0015 で永続宛先 (cwd / repo / label) を導入する設計を進める中で、kawaz から **「by-xx をファイルで管理すると、プロセスのステータスを複数ファイルの参照であちこちで扱うことになる。実態は一つなのだから」** という指摘を受けた。

確かに以下が file 分散管理だと辛い:

- session の state (sid → state / labels / cwd / repo / last-seen) を `<sid>/meta.json` に書くと、軸からの逆引き (= 「`cwd=X` の alive session 全部」) で全 meta を walk する羽目になる
...
<base>/
  ...
  msg/<msg-id>.md                # 共有メッセージ本体
                                 # (cwd/repo/label 宛は DB の queue が ref を持つ)
```
...
- 軸索引 (cwd / repo / label) が index で速い
...
CREATE TABLE sessions (
  ...
  cwd          TEXT NOT NULL,            -- 正規化済 cwd
  cwd_hash     TEXT NOT NULL,            -- SHA-256 prefix (16 char) for index
  repo_hash    TEXT,                     -- repo root hash (nullable)
  ...
  meta_json    TEXT                      -- 拡張属性 (tags 等の将来枠)
);
...
CREATE TABLE messages (
  ...
  target_kind   TEXT NOT NULL,           -- sid | cwd | repo | label
...
4. 共有宛 (cwd/repo/label) は 1.0.0 以降の新規 msg から DB queue に乗る (= 既存 msg は sid 宛 only と解釈)
