...
このディレクトリは **cmux-msg プラグイン** が Claude Code セッション間
メッセージをファイルとして保管する場所。
...
├── cmux-msg.db                      ← SQLite (sessions / labels / messages / subscribers)
...
1 つの claude セッションに対応する。cwd / repo / claude_home / tags は session に
紐付くメタ情報として `<session_id>/meta.json` と `cmux-msg.db` の `sessions` 行に
記録される。
