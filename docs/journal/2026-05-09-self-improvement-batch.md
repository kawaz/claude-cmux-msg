...
### spawn-claude-not-launching (高、再現困難)

防御的改善 + runbook 化で対応 (issue は維持):
- `docs/runbooks/spawn-troubleshooting.md` 新規作成
- spawn の signal タイムアウト警告にヒント追加 (`peers --all` / `gc` / runbook)
- session-start.ts の `main().catch` でエラー詳細を stderr に流すように変更

### slash command 検知調査 (低)

`docs/findings/2026-05-09-claude-slash-command-detection-feasibility.md` に集約:
- Claude Code hook / Plugin API / CLI には slash command 検知手段なし
- 状態ファイル (`~/.claude/projects/.../*.jsonl`) 監視は不明 (cmux-msg のスコープ超え)
- 結論: 対応しない、現状の制約として明文化

### rename / color が peers に反映されない (低)

調査結果を受けて「現状の制約」として明文化、issue は削除:
- `docs/ROADMAP.md` の「諦めた / 別件」節に追加
- `README-ja.md` の peers 行に注記 + ROADMAP リンク
...
- **「plugin update ≠ ローカル PATH 即反映」の構造的トラップ** (親 CC からの指摘):
  - `claude plugin update cmux-msg@cmux-msg` で取得しても、Claude Code を restart するまではセッション内の `cmux-msg` (zsh plugin alias 経由) は古いキャッシュバイナリ (`~/.claude/plugins/cache/cmux-msg/cmux-msg/<version>/bin/cmux-msg`) を見ている
  - 実例: 0.25.0 の `--file -` を試したつもりが、shell の cmux-msg が 0.24.0 を呼んでいて `--file -` が body 引数として join され、本文「--file -」だけが配送された
  - **対処**: cmux-msg 自身を開発中は `./bin/cmux-msg` (リポ内のラッパー、bun build 直後の `bin/cmux-msg-bin` を呼ぶ) を直接フルパスで起動する。zsh alias は信用しない
  - これは self-improvement セッション全般に出るパターン。同じ症状が出たら最初に「shell から呼ばれているバイナリのパスとバージョン」を確認する
- ショートオプション (`-h` 等) は kawaz/* 横断ルール (`~/.claude/rules/cli-design-preferences.md`) で「指示なく追加しない」と既に明記。issue の文言 (「`--help` / `-h` / `--version` / `-V` を最優先で処理して return」) を素直にコード化して `-h` を入れてしまった → kawaz から「不要、迷いとコンテキストの無駄を生む」とフィードバックを受けて 0.25.1 で削除。**issue 文言が global rule と矛盾する場合は rule を優先する**
