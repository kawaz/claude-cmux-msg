...
メッセージング:
  ...
  cmux-msg peers [--by <axis>...] [--all] [--include-dead]
                                 ...
                                 home/cwd/repo/tag:NAME を AND 結合可、--all で全 home 横断
  ...
  cmux-msg accept <filename>     メッセージを受理 (= 受け止めた、ack 受領にも使う、→ accepted/)
  cmux-msg dismiss <filename>    メッセージを破棄 (= 受け止めず却下、→ archive/)
  cmux-msg reply <filename>                    返信 & アーカイブ (本文 stdin、DR-0014)
  cmux-msg reply <filename> --text "<msg>"     一言オプション
                                 (返信→accept→archive を一気に。返信不要なら accept か dismiss)
...
async function main(): Promise<void> {
  ...
  if (!handler) {
    // DR-0019 で削除された旧コマンドには代替案を示す
    const REMOVED_HINTS: Record<string, string> = {
      spawn:
        "spawn は v0.31.0 (DR-0019) で削除されました。子 CC 起動は claude --session-id <uuid> ... を直接呼んでください。",
      stop:
        "stop は v0.31.0 (DR-0019) で削除されました。プロセス終了は OS の kill を使ってください。",
      tell:
        "tell は v0.31.0 (DR-0019) で削除されました。pane へのキー入力は hyoui 等の別 plugin に委譲します。",
      screen:
        "screen は v0.31.0 (DR-0019) で削除されました。pane の画面読み取りは hyoui 等の別 plugin に委譲します。",
    };
    const hint = REMOVED_HINTS[cmd];
    if (hint) {
