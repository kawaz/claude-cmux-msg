 ...
 *
 * - 原本は `<MSG_BASE>/.docs/v<version>/data-layout-{root,session}.md`
...
/**
 * plugin 内 docs/design/ の全ファイルの content から hash を計算する。
...
/**
 * `.docs/v<version>/` を作って plugin 内の docs/design/ からコピー。
...
export function setupLayoutDocs(opts: SetupLayoutOptions): void {
  try {
    ...
    if (!ok) return; // plugin 内 docs/design/ が無い → 何もしない
    ...
    // session: <sid>/README.md → ../.docs/latest/data-layout-session.md
    if (opts.sessionId) {
      ensureSymlink(
        path.join(opts.msgBase, opts.sessionId, "README.md"),
