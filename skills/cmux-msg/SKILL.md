...
**全コマンドの宛先指定は claude session UUID（`CMUX_MSG_SESSION_ID` の値）を使う**。
...
- SessionStart フックが `CMUX_MSG_SESSION_ID` を `$CLAUDE_ENV_FILE` に export → 以降のシェルで利用可能
...
| `CMUX_MSG_SESSION_ID` | claude session UUID。SessionStart hook が自動で export |
...
| `CMUX_MSG_PARENT_SESSION_ID` | spawn 時に自動設定される親の session_id |
