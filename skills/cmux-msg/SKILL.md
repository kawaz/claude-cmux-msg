...
`cmux-msg peers` で同一ワークスペースのピア一覧と各 session_id を確認できる。別 workspace のセッションも見たい場合は `cmux-msg peers --all-workspaces`（または `--global`）。`send` / `tell` / `screen` は session_id だけで workspace 横断的に解決される（自 workspace 優先、無ければ全 workspace 走査）。
...
# dead セッションも含めて全件表示
cmux-msg peers --all
# 全 workspace 横断で列挙 (各行に ws= が付く)
cmux-msg peers --all-workspaces

# メッセージ送信（session_id 指定。workspace 横断で自動解決）
cmux-msg send <session_id> <メッセージ>

# 同一 workspace の alive ピアにブロードキャスト (workspace は跨がない)
